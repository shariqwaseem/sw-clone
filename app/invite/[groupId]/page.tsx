...
export default function InvitePage() {
  ...
  // Fetch group info only after user is authenticated
  useEffect(() => {
    ...
    const fetchAndJoin = async () => {
      ...
      try {
        ...
        // If already a member or permission error, just redirect
        if (message.includes('PERMISSION_DENIED') || message.includes('already')) {
          localStorage.removeItem('pendingInvite');
          router.replace(`/groups/${groupId}`);
        } else {
          setError(message);
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-8 sm:p-10 shadow-lg shadow-slate-200/50">
        {error ? (
          ...
          <>
            ...
            {group && (
              <p className="mt-2 text-slate-600">
                Joining <span className="font-medium text-slate-900">{group.name}</span> as {user.displayName ?? user.email}
              </p>
            )}
          </>
        ) : null}
